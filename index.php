...
<html>
	...
	<body>
		<div class='container pt-5'>
			<h1 class='text-center text-primary'>Input data invoice</h1><hr>
			<?php
				if(isset($_POST["submit"])){
					...
					$ccity=mysqli_real_escape_string($con,$_POST["CTEL"]);
					$grand_total=mysqli_real_escape_string($con,$_POST["grand_total"]);
					
					$sql="insert into invoice (INVOICE_NO,INVOICE_DATE,CNAME,CADDRESS,CTEL,GRAND_TOTAL) values ('{$invoice_no}','{$invoice_date}','{$cname}','{$caddress}','{$ccity}','{$grand_total}') ";
					if($con->query($sql)){
						$sid=$con->insert_id;
			...
			<form method='post' action='index.php' autocomplete='off'>
				<div class='row'>
					...
					<div class='col-md-8'>
						...
						<div class='form-group'>
							<label>TEL</label>
							<input type='text' name='CTEL' required class='form-control'>
						</div>
					</div>
				</div>
				<div class='row'>
